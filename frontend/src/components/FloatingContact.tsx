...
const INITIAL_MESSAGE: Message = {
  id: 'init',
  role: 'bot',
  text: 'Hello! Welcome to Elsan Clinic. How can we assist you with your healthcare needs today?',
  options: ['Book Appointment', 'Find a Doctor', 'About Clinic']
};
...
export default function FloatingContact() {
  ...
  const handleWhatsApp = () => {
    window.open(CLINIC_INFO.whatsapp, "_blank");
  };
  ...
  const processInput = async (text: string): Promise<Message> => {
    ...
    if (lower.includes('about') || lower.includes('clinic') || lower.includes('where') || lower.includes('location')) {
      return {
        id: msgId,
        role: 'bot',
        ...
        options: ['Book Appointment', 'Main Menu']
      };
    }
    ...
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-4 font-sans">
      ...
      <div className="flex flex-col gap-4">
        ...
        <motion.button
          ...
          className="w-14 h-14 bg-[#25D366] hover:bg-[#1EBE5A] rounded-full shadow-xl flex items-center justify-center text-white transition-colors group relative"
        ...
        <motion.button
          ...
          className={`w-14 h-14 rounded-full shadow-xl flex items-center justify-center text-white transition-colors relative group ${isChatOpen ? 'bg-slate-800 hover:bg-slate-700' : 'bg-teal-600 hover:bg-teal-500'}`}
          aria-label={isChatOpen ? "Close chat" : "Open chat assistant"}
        >
          <MessageSquare size={26} className={isChatOpen ? "hidden" : "block"} />
          <svg className={`w-7 h-7 ${isChatOpen ? "block" : "hidden"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
