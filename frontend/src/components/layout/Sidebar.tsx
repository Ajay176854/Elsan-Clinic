...
const SidebarItem = ({ item, location, onClick }: { item: any, location: string, onClick?: () => void }) => {
...
export default function Sidebar({ isMobileOpen, setIsMobileOpen }: { isMobileOpen?: boolean, setIsMobileOpen?: (val: boolean) => void }) {
  ...
  return (
    <div className={cn(
      "border-r bg-white w-64 lg:w-72 shadow-sm",
      "fixed inset-y-0 left-0 z-50 transform transition-transform duration-300 ease-in-out md:relative md:translate-x-0",
      isMobileOpen ? "translate-x-0" : "-translate-x-full"
    )}>
      <div className="flex h-full flex-col gap-2">
        <div className="flex h-16 items-center border-b px-6 justify-between">
          <Link href="/" className="flex items-center gap-2 font-bold text-xl text-slate-800" onClick={() => setIsMobileOpen?.(false)}>
            <span className="text-blue-600">Elsan</span> Clinic
          </Link>
          <button onClick={() => setIsMobileOpen?.(false)} className="md:hidden text-slate-500 hover:text-slate-700">
