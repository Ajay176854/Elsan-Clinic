...
export default function PublicViews({ view, onNavigate }: { view: ViewState, onNavigate: (v: ViewState) => void }) {
  if (view === 'about') return <AboutView onNavigate={onNavigate} />;
  if (view === 'services') return <ServicesView onNavigate={onNavigate} />;
  if (view === 'doctors') return <DoctorsView onNavigate={onNavigate} />;
...
function AboutView({ onNavigate }: { onNavigate: (v: ViewState) => void }) {
  return (
...
function DoctorsView({ onNavigate }: { onNavigate: (v: ViewState) => void }) {
  return (
...
const getServiceImage = (title: string) => {
  ...
  return map[title] || 'https://images.unsplash.com/photo-1538108149393-cebb47cbdc12?w=800&q=80';
};

function ServicesView({ onNavigate }: { onNavigate: (v: ViewState) => void }) {
            ...
            <div className="p-6 md:p-8 space-y-6">
              <div className="flex items-start gap-4">
                <div className="p-3 bg-blue-50 text-blue-600 rounded-xl shrink-0">
                   {React.createElement(ICON_MAP[selectedService.iconName] || Activity, { size: 28 })}
                </div>
                <div>
